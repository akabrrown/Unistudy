...
async function logAuditAction(adminId, actionType, targetId, targetType, details = {}) {
    try {
        await supabase_1.supabaseAdmin.from('admin_audit_logs').insert({
            admin_id: adminId,
...
router.get('/overview', async (req, res) => {
    try {
        const [{ count: totalUsers }, { count: newUsers }, { count: activePaid }, { count: activeRequests }] = await Promise.all([
            supabase_1.supabaseAdmin.from('profiles').select('*', { count: 'exact', head: true }),
            supabase_1.supabaseAdmin.from('profiles').select('*', { count: 'exact', head: true }).gte('created_at', new Date(new Date().setHours(0, 0, 0, 0)).toISOString()),
            supabase_1.supabaseAdmin.from('subscriptions').select('*', { count: 'exact', head: true }).eq('status', 'active'),
            supabase_1.supabaseAdmin.from('ai_usage_logs').select('*', { count: 'exact', head: true }).gte('created_at', new Date(new Date().setHours(0, 0, 0, 0)).toISOString())
        ]);
        // Aggregate provider usage
        const { data: providers } = await supabase_1.supabaseAdmin.from('all_providers_quota').select('*');
        res.json({
            kpis: {
...
router.get('/users/:userId/details', async (req, res) => {
    try {
        ...
        let activityData = [];
        try {
            const { data } = await supabase_1.supabaseAdmin.from('admin_audit_logs').select('*').eq('target_id', userId).order('created_at', { ascending: false }).limit(10);
            if (data)
                activityData = data;
        }
        catch (e) {
        ...
        const [coursesRes, aiTodayRes, aiMonthRes] = await Promise.all([
            supabase_1.supabaseAdmin.from('courses').select('id, course_code, title').eq('user_id', userId),
            supabase_1.supabaseAdmin.from('ai_request_log').select('*', { count: 'exact', head: true }).eq('user_id', userId).gte('created_at', startOfToday),
            supabase_1.supabaseAdmin.from('ai_request_log').select('*', { count: 'exact', head: true }).eq('user_id', userId).gte('created_at', startOfMonth)
        ]);
        res.json({
...
router.patch('/users/:userId/role', async (req, res) => {
    ...
    try {
        ...
        if (error)
            throw error;
        await logAuditAction(req.user.id, role === 'suspended' ? 'USER_SUSPENDED' : (role === 'admin' ? 'USER_ROLE_CHANGED_ADMIN' : 'USER_ROLE_CHANGED_STUDENT'), userId, 'user', { role, suspendReason });
...
router.get('/content', async (req, res) => {
    try {
        ...
        const { data: lectures, error } = await supabase_1.supabaseAdmin
            .from('lectures')
...
router.delete('/content/:id', async (req, res) => {
    ...
    try {
        ...
        if (error)
            throw error;
        await logAuditAction(req.user.id, 'CONTENT_DELETED', contentId, 'lecture', {});
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
router.get('/ai-usage', async (req, res) => {
    try {
        ...
        const { data: dailyLogs } = await supabase_1.supabaseAdmin
            .from('ai_request_log')
        ...
        const { data: monthlyLogs } = await supabase_1.supabaseAdmin
            .from('ai_request_log')
            .select('provider, tokens_cost')
            .gte('called_at', startOfMonth);
        const tokensPerProvider = {};
        if (monthlyLogs) {
        ...
        const { data: todayLogs } = await supabase_1.supabaseAdmin
            .from('ai_request_log')
        ...
        const topUsers = Object.values(usersMap)
            .map((u) => ({ ...u, features: Array.from(u.features).join(', ') }))
...
router.post('/ai-usage/toggle-fallback', async (req, res) => {
...
router.get('/subscriptions', async (req, res) => {
    try {
        ...
        const { data: subs } = await supabase_1.supabaseAdmin
            .from('subscriptions')
        ...
        const { count: churnThisMonth } = await supabase_1.supabaseAdmin.from('subscriptions')
            .select('*', { count: 'exact', head: true })
            .in('status', ['canceled', 'past_due', 'unpaid'])
            .gte('updated_at', startOfThisMonth);
        const { count: churnLastMonth } = await supabase_1.supabaseAdmin.from('subscriptions')
            .select('*', { count: 'exact', head: true })
            .in('status', ['canceled', 'past_due', 'unpaid'])
        ...
        let paymentLogs = [];
        try {
            const { data } = await supabase_1.supabaseAdmin.from('paystack_webhook_logs').select('id, event_type, user_id, amount, reference, status, created_at, profiles(email)').order('created_at', { ascending: false }).limit(50);
            if (data)
                paymentLogs = data;
        }
        catch (e) {
...
router.post('/subscriptions/override', async (req, res) => {
    const { userId, newPlan, expiryDays } = req.body;
    try {
        const expiry = new Date();
        expiry.setDate(expiry.getDate() + (expiryDays || 30));
        // Update or Insert Subscription
        await supabase_1.supabaseAdmin.from('subscriptions').upsert({
            user_id: userId,
            plan_id: newPlan,
            status: 'active',
        ...
        await supabase_1.supabaseAdmin.from('profiles').update({ plan: newPlan }).eq('id', userId);
