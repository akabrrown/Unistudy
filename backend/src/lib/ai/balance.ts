...
export async function deductFromPool(provider: ProviderName, amount: number) {
  ...
  if (config.pool_type === 'free_rate_limit') return true;
...
export async function checkAndAlert(provider: ProviderName) {
  ...
  if (status.should_disable && !status.is_disabled) {
    if (!(await hasUnresolvedAlert('auto_disable'))) {
      ...
      await supabaseAdmin.from('platform_ai_balance').update({ is_disabled: true }).eq('provider', provider);
    }
  } 
    ...
    if (!(await hasUnresolvedAlert('restrict_free'))) {
      ...
      await supabaseAdmin.from('platform_ai_balance').update({ is_fallback_active: true }).eq('provider', provider);
    }
  }
...
export async function getAllProviderStatuses() {
  const { data } = await supabaseAdmin.from('platform_ai_balance').select('*').order('provider');
  return data || [];
}
