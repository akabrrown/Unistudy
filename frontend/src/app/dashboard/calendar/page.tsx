...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, Plus, Trash2, Loader2, BookOpen, Clock, FileWarning, RefreshCcw } from 'lucide-react';
import { toast } from 'sonner';
import { GenerateScheduleWizard } from '@/components/planner/GenerateScheduleWizard';
...
export default function CalendarPage() {
  ...
  // Re-fetch events when switching back to the calendar view
  useEffect(() => {
    if (view === 'calendar') {
      fetchEvents();
    }
  ...
  const handleDeleteTimetable = async () => {
    if (!confirm('Are you sure you want to delete your entire generated study timetable? This will remove all planned sessions from your calendar.')) return;
    setIsDeleting(true);
    try {
      const res = await fetch('/api/planner/template', { method: 'DELETE' });
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {template && (
            <Button variant="outline" className="border-red-500/30 text-red-500 hover:bg-red-500/10 hover:text-red-600" disabled={isDeleting} onClick={handleDeleteTimetable}>
              {isDeleting ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Trash2 className="w-4 h-4 mr-2" />}
              Delete Timetable
            </Button>
          )}
          <GenerateScheduleWizard onSuccess={() => { fetchEvents(); fetchTemplate(); setView('timetable'); }} />
      ...
      <div className="flex items-center gap-2">
        <div className="flex bg-muted/50 p-1 rounded-lg border border-border">
          ...
          >
            <CalendarIcon className="w-4 h-4 mr-2" /> Live Calendar
          </Button>
          <Button
          ...
          >
            <LayoutGrid className="w-4 h-4 mr-2" /> Weekly Timetable
          </Button>
        </div>
      ...
      {view === 'timetable' ? (
        <WeeklyTimetable template={template} onUpdate={() => { fetchEvents(); fetchTemplate(); setView('calendar'); }} />
      ) : (
