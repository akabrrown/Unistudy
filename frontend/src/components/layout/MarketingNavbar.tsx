...
export function MarketingNavbar() {
  ...
  return (
    <nav className="sticky top-0 z-40 bg-[#FAF8FF]/90 dark:bg-[#0F0C29]/90 backdrop-blur border-b border-[#D1C4E9]/60 dark:border-white/10 px-8 h-16 flex items-center justify-between transition-colors">
      ...
      {/* Mobile hamburger */}
      <button
        className="md:hidden p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#5B2D8E]"
      ...
      {/* Mobile menu overlay */}
      {mobileMenuOpen && (
        <div className="fixed z-50 top-16 left-0 w-full bg-[#FAF8FF] dark:bg-[#0F0C29] border-b border-[#D1C4E9]/60 dark:border-white/10 md:hidden">
          <div className="flex flex-col p-4 space-y-3">
            {links.map((link) => (
              <Link key={link.label} href={link.href} className="text-[14px] text-[#6B5A8A] dark:text-[#B39DDB] hover:text-[#5B2D8E] dark:hover:text-white transition-colors" onClick={() => setMobileMenuOpen(false)}>
                {link.label}
              </Link>
            ))}
          </div>
